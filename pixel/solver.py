...
class Solver(object):
  ...
  def sample(self, sess, mu=1.1, step=None):
    ...
    gen_hr_imgs = np.zeros((self.batch_size, sz_hr, sz_hr, 3), dtype=np.float32)
    ...
    #
    save_samples(np_lr_imgs, self.samples_dir + '/lr_' + str(mu*10) + '_' + str(step) + '.png')
    save_samples(np_hr_imgs, self.samples_dir + '/hr_' + str(mu*10) + '_' + str(step) + '.png')
    save_samples(gen_hr_imgs, self.samples_dir + '/generate_' + str(mu*10) + '_' + str(step) + '.png')
